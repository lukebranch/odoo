...
#         dr0 = self.leads_dry_run.search_read(cr, uid, [('lead_id', '=', self.lead0)], fields)[0]
#         dr1 = self.leads_dry_run.search_read(cr, uid, [('lead_id', '=', self.lead1)], fields)[0]
#         dr2 = self.leads_dry_run.search_read(cr, uid, [('lead_id', '=', self.lead2)], fields)

#         self.assertEqual(dr0['section_id'][0], self.section0, 'dry run failed')
#         self.assertEqual(dr1['section_id'][0], self.section1, 'dry run failed')
#         self.assertEqual(dr2, [], 'dry run failed')

#         self.assertEqual(dr0['user_id'][0], self.salesmen0, 'dry run failed')
#         self.assertEqual(dr1['user_id'][0], self.salesmen1, 'dry run failed')

#         ldr = self.leads_dry_run.search_count(cr, uid, [], None)

#         self.assertEqual(ldr, 2, 'wrong number of dry run created')
