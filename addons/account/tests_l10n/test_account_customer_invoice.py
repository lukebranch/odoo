...
class TestAccountCustomerInvoive(AccountTestUsers):
    ...
    def test_customer_invoice(self):
        ...
        account_cash = self.account_model.search([('internal_type', '=', 'liquidity')])[0]
        bank_journal = self.journal_model.search([('type', '=', 'bank')])[0]
        # I totally pay the Invoice
        pay = self.account_invoice_customer0.pay_and_reconcile(
            10050.0, account_cash.id,
            datetime.date.today(), bank_journal.id,
        )
