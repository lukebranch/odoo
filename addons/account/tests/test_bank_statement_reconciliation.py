...
class TestBankStatementReconciliation(TransactionCase):
    ...
    def create_invoice(self, amount):
        ...
        res = self.AccountInvoice.onchange_partner_id('out_invoice', self.partner_agrolait.id)['value']
        ...
        vals.update(res)
        invoice = self.AccountInvoice.create(vals)
    ...
    def create_statement_line(self, st_line_amount):
        journal = self.AccountBankStatement.with_context(journal_type='bank')._default_journal()
        bank_stmt = self.AccountBankStatement.create({'journal_id': journal.id})

        bank_stmt_line = self.AccountBankStatementLine.create({
            'name': '_',
