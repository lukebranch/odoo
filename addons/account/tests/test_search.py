...
class TestSearch(AccountTestUsers):
    ...
    def test_name_search(self):
        account_type = self.AccountAccountType.search([], limit=1)
        self.atax = self.AccountAccount.create(dict(
            name="Tax Received",
            code="X121",
            user_type=account_type.id,
        )).id, "X121 Tax Received"

        self.apurchase = self.AccountAccount.create(dict(
            name="Purchased Stocks",
            code="X1101",
            user_type=account_type.id,
        )).id, "X1101 Purchased Stocks"

        self.asale = self.AccountAccount.create(dict(
            name="Product Sales",
            code="XX200",
            user_type=account_type.id,
        ...
        atax_ids = self.AccountAccount.name_search(name="Tax", operator='ilike', args=[('id', 'in', self.all_ids)])
        ...
        atax_ids = self.AccountAccount.name_search(name="Tax", operator='not ilike', args=[('id', 'in', self.all_ids)])
        ...
        apur_ids = self.AccountAccount.name_search(name='Purchased Stocks', operator='ilike', args=[('id', 'in', self.all_ids)])
        ...
        apur_ids = self.AccountAccount.name_search(name='Purchased Stocks', operator='not ilike', args=[('id', 'in', self.all_ids)])
        ...
        asale_ids = self.AccountAccount.name_search(name='Product Sales', operator='ilike', args=[('id', 'in', self.all_ids)])
        ...
        asale_ids = self.AccountAccount.name_search(name='Product Sales', operator='not ilike', args=[('id', 'in', self.all_ids)])
        ...
        asale_ids = self.AccountAccount.name_search(name='XX200', operator='ilike', args=[('id', 'in', self.all_ids)])
