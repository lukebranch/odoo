...
class sale_quote(http.Controller):
    ...
    def confirm(self, order_id):
        ...
        if tx and tx.state == 'pending':
            ...
            request.registry['account.invoice'].pay_and_reconcile(cr, SUPERUSER_ID, invoice.id, tx.amount, journal.default_credit_account_id.id,
                                                                  period_id, journal.id, False, False, False, context=context)
