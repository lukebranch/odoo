    ...
    def process_coda(self, cr, uid, codafile=None, journal_id=False, context=None):
        ...
        for line in recordlist:
            if not line:
                ...
                if not period_id and len(period_id) == 0:
                    raise UserError(_('Error') + 'R0002: ' + _("The CODA Statement New Balance date doesn't fall within a defined Accounting Period! Please create the Accounting Period for date %s for the company %s.") % (statement['balance_end_realDate'], statement['journal_id'].company_id.name))
        ...
        for i, statement in enumerate(statements):
            ...
            if not(statement.get('period_id')):
                raise UserError(_('Error') + ' R3006: ' + _(' No transactions or no period in coda file !'))
            statement_data = {
