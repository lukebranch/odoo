...
class AccountContractDashboard(http.Controller):
    ...
    def account_contract_dashboard(self, **kw):
        ...
        start_date = datetime.strptime(kw.get('start_date'), DEFAULT_SERVER_DATE_FORMAT) if kw.get('start_date') else default_start_date
        end_date = datetime.strptime(kw.get('end_date'), DEFAULT_SERVER_DATE_FORMAT) if kw.get('end_date') else default_end_date

        href_post_args = 'start_date=%s&end_date=%s&' % (start_date.strftime(DEFAULT_SERVER_DATE_FORMAT), end_date.strftime(DEFAULT_SERVER_DATE_FORMAT))
    ...
    def salesman_dashboard(self, **kw):
        ...
        """)  # we could also use distinct(user_id) on account_invoice
        sql_results = request.cr.dictfetchall()
        for entry in sql_results:
            salesman_ids.append(request.env['res.users'].browse(entry['id']))
    ...
    def get_values_salesman(self, salesman_id, start_date=None):
        ...
        for contract in invoice_line_ids_starting_last_month.mapped('account_analytic_id'):
            ...
            if previous_il_ids:
                ...
                # cast in int is to avoid rounding precision in python (ex : 10.0000000006 < 10)
        ...
        for invoice_id in current_invoice_ids:
            invoice_nrr = sum([x.price_subtotal_signed for x in invoice_id.invoice_line if x.mrr == 0])
            if invoice_nrr > 0:
    ...
    def calculate_stat_aggregate(self, stat_type, start_date, end_date, filtered_contract_template_ids=None):
        ...
        if stat_type == 'net_revenue':
            if not filtered_contract_template_ids:
                request.cr.execute("""
                    SELECT SUM(line.price_subtotal_signed) AS sum
