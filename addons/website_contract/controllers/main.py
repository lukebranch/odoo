...
class website_contract(http.Controller):
    ...
    def contract(self, account_id, uuid='', message='', message_class=''):
        ...
        dummy, action = request.env['ir.model.data'].get_object_reference('account_analytic_analysis', 'action_account_analytic_overdue_all')
    ...
    def payment(self, account_id, uuid=None, **post):
        ...
        if payment_method:
            ...
            if tx.html_3ds:
                return tx.html_3ds
            account.sudo().reconcile_pending_transaction(account.id, tx, new_invoice)
            if tx.state == 'done':
                account.sudo().send_success_mail(tx, new_invoice)
                get_param = self.payment_succes_msg
            else:
                get_param = self.payment_fail_msg
