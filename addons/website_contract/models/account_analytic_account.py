...
class account_analytic_account(models.Model):
    ...
    def change_subscription(self, new_template_id):
        ...
        for line in self.recurring_invoice_line_ids:
            if line.product_id in [tmp_line.product_id for tmp_line in self.template_id.recurring_invoice_line_ids]:
                mand_lines_qty[line.product_id.id] = line.quantity
        ...
        rec_lines_to_add = [(0, 0, {
            ...
            'name': tmp_line.name,
            'quantity': mand_lines_qty[tmp_line.product_id.id] if tmp_line.product_id.id in mand_lines_qty else tmp_line.quantity,
            'price_unit': self.pricelist_id.with_context({'uom': tmp_line.uom_id.id}).price_get(tmp_line.product_id.id, 1)[self.pricelist_id.id],
            'analytic_account_id': self.id,
    ...
    @api.one
    def _do_payment(self, payment_method, invoice):
    ...
    def reconcile_pending_transaction(self, contract_id, tx, invoice):
        ...
        if tx.state == 'done':
            ...
            contract.write({'recurring_next_date': new_date.strftime('%Y-%m-%d'), 'state': 'open', 'date': False})
        elif tx.state == 'error':
                ...
                for contract in self.with_context(dict(self.env.context, company_id=company_id, force_company=company_id)).browse(ids):
                    ...
                    if contract.template_id and contract.template_id.payment_mandatory and contract.recurring_total and automatic:
                        try:
                            ...
                            tx = contract._do_payment(payment_method, new_invoice)[0]
