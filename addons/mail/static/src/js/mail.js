...
var MessageCommon = Widget.extend({
    /**
     * ------------------------------------------------------------
    ...
    format_data: function () {
        ...
        this.date = this.date ? time.str_to_datetime(this.date) : false;
    ...
    */
    ...
    browse_message: function (options) {
        ...
        if (!options._go_thread_wall) {
            ...
            for (i in this.options.root_thread.messages) {
                res=this.options.root_thread.messages[i].browse_message(options);
                if (res) return res;
            }
        }
        ...
        for (i in this.thread.messages) {
            if (this.thread.messages[i].thread) {
                res=this.thread.messages[i].browse_message(options);
                if (res) return res;
            }
        }
...
var ThreadComposeMessage = MessageCommon.extend({
    ...
    */
    ...
    do_check_attachment_upload: function () {
        if (_.find(this.attachment_ids, function (file) {return file.upload;})) {
            this.do_warn(qweb.render('mail.error_upload'), qweb.render('mail.error_upload_please_wait'));
            return false;
        } else {
            return true;
        }
    ...
    on_toggle_quick_composer: function (event) {
        ...
        $.when(suggested_partners).pipe(function () {
    ...
    compute_emails_from: function () {
        ...
        if (this.parent_thread.parent_message) {
            ...
            if(parent_message){
                messages = [parent_message].concat(parent_message.get_childs());
            }
 ...
 * - - expandable
 */
var ThreadExpandable = MessageCommon.extend({
    ...
    reinit: function () {
        var $render = $(qweb.render('mail.thread.expandable', {'widget': this}));
...
var ThreadMessage = MessageCommon.extend({
    ...
    create_thread: function () {
        ...
        /*create thread*/
        this.thread = new Thread(this, this, {
                'domain': this.domain,
                'context':{
    ...
    */
    display_vote: function () {
        var vote_element = qweb.render('mail.thread.message.vote', {'widget': this});
    ...
    */
    instantiate_compose_message: function () {
        ...
        if (!this.compose_message) {
            this.compose_message = new ThreadComposeMessage(this, this, {
    ...
    */
    get_childs: function (nb_thread_level) {
        ...
        if (isNaN(nb_thread_level) || nb_thread_level>0) {
            _(this.messages).each(function (val) {
                if (val.thread) {
    ...
    */
    ...
    create_message_object: function (data) {
        ...
        if (data.type=='expandable') {
            message = new ThreadExpandable(self, data, {'context':{
            ...
            message = new ThreadMessage(self, data, {'context':{
    ...
    switch_new_message: function (records, dom_insert_after) {
        var self = this;
        dom_insert_after = typeof dom_insert_after == 'object' ? dom_insert_after : false;
    ...
    message_to_expandable: function (message) {
        ...
        msg_up = msg_up.hasClass('oe_msg_expandable') ? _.find( this.messages, function (val) { return val.$el[0] == msg_up[0]; }) : false;
        msg_down = msg_down.hasClass('oe_msg_expandable') ? _.find( this.messages, function (val) { return val.$el[0] == msg_down[0]; }) : false;
        ...
        if ( msg_up && msg_up.type == "expandable" && msg_down && msg_down.type == "expandable") {
            ...
            var expandable = new ThreadExpandable(this, {
                'model': message.model,
 ...
 */
var RecordThread = form_common.AbstractField.extend({
...
var MailWall = Widget.extend({
    ...
    init: function (parent, action) {
        ...
        this.defaults = {};
        for (key in this.action.context.params) {
    ...
    start: function () {
        ...
        this.bind_events();
        this.load_searchview(this.defaults);
