            ...
            if i_line.product_id.type != 'service':
                ...
                for line in res:
                    if line.get('invl_id', 0) == i_line.id and a == line['account_id']:
                        ...
                        if i_line.product_id.cost_method != 'standard' and i_line.purchase_line_ids:
                            #for average/fifo/lifo costing method, fetch real cost price from incomming moves
                            stock_move_obj = self.pool.get('stock.move')
                            valuation_stock_move = stock_move_obj.search(cr, uid, [('purchase_line_id', 'in', [x.id for x in i_line.purchase_line_ids])], limit=1, context=context)
