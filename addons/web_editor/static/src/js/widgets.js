...
var widgets = web_editor.widgets = {};
...
define(['summernote/openerp'], function () {
    ...
    widgets.Dialog = openerp.Widget.extend({
        events: {
     ...
     */
    widgets.alt = widgets.Dialog.extend({
     ...
     */
    widgets.MediaDialog = widgets.Dialog.extend({
        ...
        start: function () {
            ...
            this.imageDialog = new widgets.ImageDialog(this, this.media, this.options);
            this.imageDialog.appendTo(this.$("#editor-media-image"));
            this.iconDialog = new widgets.FontIconsDialog(this, this.media, this.options);
            this.iconDialog.appendTo(this.$("#editor-media-icon"));
            this.videoDialog = new widgets.VideoDialog(this, this.media, this.options);
    ...
    var IMAGES_ROWS = 2;
    widgets.ImageDialog = openerp.Widget.extend({
        ...
        form_submit: function (event) {
            ...
            window[callback] = function (attachments, error) {
                ...
                for (var i=0; i<attachments.length; i++) {
                    self.file_selected(attachments[i]['local_url'], error);
                }
            };
        },
        ...
        try_remove: function (e) {
            ...
            return openerp.jsonRpc('/web_editor/attachment/remove', 'call', {'ids': [id]}).then(function (prevented) {
    ...
    widgets.getCssSelectors = function(filter) {
        var css = [];
    ...
    function computeFonts() {
        _.each(widgets.fontIcons, function (data) {
            data.icons = _.map(widgets.getCssSelectors(data.parser), function (css) {
     ...
     */
    widgets.fontIcons = [{'base': 'fa', 'parser': /(?=^|\s)(\.fa-[0-9a-z_-]+::?before)/i}];
     ...
     */
    widgets.FontIconsDialog = openerp.Widget.extend({
        ...
        renderElement: function() {
            this.iconsParser = widgets.fontIcons;
            this.icons = _.flatten(_.map(widgets.fontIcons, function (data) {
                    return data.icons;
                }));
            this._super();
        },
        ...
        search: function (needle) {
            ...
            if (needle) {
                var parser = [];
                var fontIcons = widgets.fontIcons, fontIcon;
            ...
            this.$('div.font-icons-icons').html(
                openerp.qweb.render('web_editor.dialog.font-icons.icons', {'iconsParser': iconsParser}));
        },
        /**
        ...
        getFont: function (classNames) {
            ...
            var fontIcons = widgets.fontIcons, fontIcon, className;
     ...
     */
    widgets.VideoDialog = openerp.Widget.extend({
    ...
    /* ----- EDITOR: LINK & MEDIA ---- */
    widgets.LinkDialog = widgets.Dialog.extend({
