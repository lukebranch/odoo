(function () {
    ...
    web_editor.EditorBar.include({
        init: function() {
            ...
            window.snippets = this.snippets = new snippet.BuildingBlock(this, $editable);
    ...
    snippet.BuildingBlock = openerp.Widget.extend({
        ...
        start: function() {
            ...
            $.summernote.core.dom.isNotBreakable = function (node) {
                return _isNotBreakable(node) || $(node).is('div') || snippet.globalSelector.is($(node));
            };
        ...
        bind_snippet_click_editor: function () {
            ...
            $(document).on('click', '*', function (event) {
                ...
                if (!snippet.globalSelector.is($target)) {
                    $target = snippet.globalSelector.closest($target);
                }
        ...
        clean_for_save: function () {
            var self = this;
            var options = snippet.options;
            var template = self.templateOptions;
            for (var k in template) {
                var Option = options[template[k]['option']];
                if (Option && Option.prototype.clean_for_save !== snippet.dummy) {
        ...
        create_overlay: function ($snippet) {
            if (typeof $snippet.data("snippet-editor") === 'undefined') {
                ...
                $snippet.data("snippet-editor", new snippet.Editor(this, $snippet));
            }
                ...
                stop: function(ev, ui){
                    ...
                    if (dropped) {
                        ...
                        setTimeout(function () {
                            ...
                            snippet.start_animation(true, $target);
        ...
        activate_overlay_zones: function($targets){
            ...
            $targets.each(function () {
                ...
                if (!$target.data('overlay')) {
                    var $zone = $(openerp.qweb.render('web_editor.snippet_overlay'));
    ...
    snippet.Editor = openerp.Class.extend({
        ...
        load_style_options: function () {
            ...
            _.each(this.BuildingBlock.templateOptions, function (val, option_id) {
                ...
                var Editor = snippet.options[option] || snippet.Option;
        ...
        get_parent_block: function () {
            ...
            var $parent = snippet.globalSelector.closest(this.$target.parent());
            if ($parent.length) {
