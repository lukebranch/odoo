...
openerp.jsonRpc('/web/dataset/call', 'call', {
        ...
        'args': ['web_editor.colorpicker', openerp.web_editor.get_context()]
    }).done(function (data) {
...
define(['summernote/openerp'], function () {
    ...
    renderer.createPalette = function ($container, options) {
        ...
        if (!openerp.qweb.has_template('web_editor.colorpicker')) {
            return;
        }

        var $color = $container.find('.note-color');
        var html = openerp.qweb.render('web_editor.colorpicker');
    ...
    eventHandler.dialog.showLinkDialog = function ($editable, $dialog, linkInfo) {
        ...
        var editor = new web_editor.widgets.LinkDialog($editable, linkInfo);
    ...
    eventHandler.dialog.showImageDialog = function ($editable) {
        ...
        var editor = new web_editor.widgets.MediaDialog($editable, dom.isImg(r.sc) ? r.sc : null);
        editor.appendTo(document.body);
    ...
    $.summernote.pluginEvents.alt = function (event, editor, layoutInfo, sorted) {
        ...
        new web_editor.widgets.alt($editable, media).appendTo(document.body);
    };
    ...
    dom.isImgFont = function (node) {
        ...
        if (node && (nodeName === "SPAN" || nodeName === "I") && className.length) {
            ...
            for (var k=0; k<web_editor.widgets.fontIcons.length; k++) {
                if (_.intersection(web_editor.widgets.fontIcons[k].icons, classNames).length) {
                    return true;
                }
            }
        }
    ...
    eventHandler.attach = function (oLayoutInfo, options) {
        ...
        oLayoutInfo.editor.on('dblclick', 'img, .media_iframe_video, span.fa, i.fa, span.fa', function (event) {
            if (!$(event.target).closest(".note-toolbar").length) { // prevent icon edition of top bar for default summernote
                new web_editor.widgets.MediaDialog(oLayoutInfo.editor, event.target).appendTo(document.body);
            }
        });
