(function () {
    ...
    var cache = {};
    var rulesCache = [];
    web_editor.getMatchedCSSRules = function getMatchedCSSRules(a) {
    ...
    web_editor.font_to_img = function ($editable) {
        $(".fa", $editable).each(function () {
            var $font = $(this);
            var content;
            _.find(web_editor.widgets.fontIcons, function (font) {
                return _.find(web_editor.widgets.getCssSelectors(font.parser), function (css) {
            ...
            if (content) {
                ...
                var src = _.str.sprintf('/web_editor/font_to_img/%s/%s/'+$font.width(), window.encodeURI(content), window.encodeURI($font.css("color").replace(/\s/g, '')));
    ...
    web_editor.img_to_font = function ($editable) {
    ...
    web_editor.class_to_style = function ($editable) {
        if (!rulesCache.length) {
            web_editor.getMatchedCSSRules($editable[0]);
        }
        var selector = _.map(rulesCache, function (a) { return a.selector;}).join(",");
        $editable.find(selector).each(function () {
            var $target = $(this);
            var css = web_editor.getMatchedCSSRules(this);
    ...
    web_editor.style_to_class = function ($editable) {
        web_editor.getMatchedCSSRules($editable[0]);
        var classes = [];
