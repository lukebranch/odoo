...
class ir_http(orm.AbstractModel):
    ...
    def _dispatch(self):
        ...
        if request.website_enabled:
            ...
            if first_pass:
                url_lang = not func and path[1] # EN_GB
                #url_lang = not func and self.get_nearest_lang(path[1]) in langs and path[1] # EN_GB
                nearest_lang = url_lang and self.get_nearest_lang(path[1]) # EN_US
                #preferred_lang = cook_lang or self.get_nearest_lang(request.lang) or request.website.default_lang_code
                ...
                if ((url_lang and (url_lang != request.lang or url_lang == request.website.default_lang_code))
                        #or (not url_lang and request.lang != request.website.default_lang_code)
        ...
        #if cook_lang != request.lang:
        if request.website_enabled and cook_lang != request.lang:
