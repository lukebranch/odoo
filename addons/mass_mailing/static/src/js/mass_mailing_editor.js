(function () {
    ...
    web_editor.snippet.options["width-x"] = web_editor.snippet.Option.extend({
        start: function () {
    ...
    var _set_value = window.top.openerp[callback+"_updown"];
    window.top.openerp[callback+"_updown"] = function (value, fields_values, field_name) {
        ...
        if(value !==_val) {
            ...
            if (value.indexOf('on_change_model_and_list') === -1) {
                ...
                if (editor_enable) {
                    openerp.web_editor.editor_bar.display_theme_from_html();
        ...
        if (fields_values.mailing_model) {
            openerp.web_editor.editor_bar.get_snippet_template(fields_values.mailing_model);
            if (value.indexOf('on_change_model_and_list') !== -1) {
