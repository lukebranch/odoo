...
class wizard_user(osv.osv_memory):
    ...
    def _retrieve_user(self, cr, uid, wizard_user, context=None):
        ...
        return user_ids and wizard_user.write({'user_id': user_ids[0]}) or False
    ...
    def _send_email(self, cr, uid, wizard_user, context=None):
        ...
        template_id = self.pool['ir.model.data'].xmlid_to_res_id(cr, uid, 'portal.welcome_mail_portal_user')
        if template_id:
            return self.pool['mail.template'].send_mail(cr, uid, template_id, wizard_user.id, force_send=True, context=context)
