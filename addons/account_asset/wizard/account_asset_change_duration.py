...
class asset_modify(models.TransientModel):
    ...
    def _get_asset_method_time(self):
        if self.ids and len(self.ids) == 1 and self._context.get('active_id'):
            asset = self.env['account.asset.asset'].browse(self._context.get('active_id'))
            return {self.ids[0]: asset.method_time}
        else:
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        """
        result = super(asset_modify, self).fields_view_get(view_id, view_type, toolbar=toolbar, submenu=submenu)
    ...
    def default_get(self, fields):
        ...
        """
        res = super(asset_modify, self).default_get(fields)
