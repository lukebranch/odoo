...
class website_event(http.Controller):
    ...
    def events(self, page=1, **searches):
        ...
        types = Event.read_group(domain, ["id", "type"], groupby="type", orderby="type")
        type_count = Event.search_count(domain)
        types.insert(0, {
        ...
        country_id_count = Event.search_count(domain)
        countries.insert(0, {
    ...
    def add_event(self, event_name="New Event", **kwargs):
        event = self._add_event(event_name)
        return request.redirect("/event/%s/register?enable_editor=1" % slug(event))

    def _add_event(self, event_name=None):
        if not event_name:
        ...
        return request.env['event.event'].create(vals)
    ...
    def get_country_events(self, **post):
        ...
        event_ids = None
        if country_code:
            country_id = Country.search([('code', '=', country_code)])
            event_ids = Event.search(['|', ('address_id', '=', None), ('country_id.code', '=', country_code), ('date_begin', '>=', '%s 00:00:00' % fields.Date.today()), ('state', '=', 'confirm')], order="date_begin")
        if not event_ids:
            event_ids = Event.search([('date_begin', '>=', '%s 00:00:00' % fields.Date.today()), ('state', '=', 'confirm')], order="date_begin")
